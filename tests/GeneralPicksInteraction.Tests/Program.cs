...
if (!args.Contains("--sql") && !args.Contains("--fixture-sql") && !args.Contains("--alias-sql")) return;
...
if (args.Contains("--alias-sql"))
{
    await OfficialAliasSettlementTests.Run(configuration, connectionString);
    return;
}
