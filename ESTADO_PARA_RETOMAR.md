...
## Laboratorios G, H e I: evidencia, enlaces oficiales y resúmenes

- Informe completo: `docs/labs-g-h-i-auditoria-2026-09-21.md`. Se añadieron conclusiones, muestra útil, cobertura y siguiente paso a cada lab. No se entrenaron/promovieron modelos ni se cambió la regla productiva del 3%.
- **G sigue sin artefacto activo**. Aproximadamente 91% de sus pendientes carece del ID que exige el worker G (131.195 sin ID frente a 13.356 con ID en la lectura). Para el rango 15–30/09 se verificaron 69.201 observaciones, 370 partidos, 123 con algún resultado y 0 aprobaciones. El ancla de mercado con reliability=0 dejó de puntuarse como modelo G. Brier/log-loss sólo comparan pares binarios válidos; volumen no promueve el bot. La web abre con última semana y próximos partidos; ampliar a todo el histórico sigue siendo costoso.
- **H v1.1: 10/10 primeras aprobaciones liquidadas**, 6 ganadas/4 perdidas, +0,585u sobre 5u = **+11,7%**. Error modelo/mercado 0,24554/0,25081 en diez pares: señal inicial favorable, muestra muy pequeña. La v1.0 tiene una observación sin córners disponibles y se muestra aparte. Para calibrar hay que estudiar también rechazadas deduplicadas y reservar fechas futuras, no usar sólo las diez aprobadas.
- Se recuperaron cinco resultados H mediante alias explícitos de Dundee United/Utd, Coventry City/Coventry, Bolton Wanderers/Bolton, West Ham United/West Ham y Bayern Munich/München. El catálogo mantiene nombres completos y no dispara una reescritura global. H/I exigen IDs de competición verificados para England Premier League (39), Championship (40), Scotland Premiership (179) y UEFA Champions League (2), evitando un alias global ambiguo de «Premier League».
- **I, 30 días: 483 primeras aprobaciones, 248 resueltas**, 127 ganadas/121 perdidas, −17,76u sobre 248u = **−7,16%**. Goles: 163 resueltas/−8,20%; córners: 85/−5,18%. Quedan 224 sin enlace y 11 futuras. Esto describe la parte resuelta; todavía falta cobertura. SignalScore no es probabilidad calibrada y no existe CLV.
- H/I ahora resuelven identidad al leer sin reescribir snapshots. I conserva ID original y expone ID recuperado; rechaza ambigüedad, competiciones incorrectas y resultados futuros. MatchHistory tiene fecha DATE sin hora; el control previo al inicio usa el kickoff UTC auditado, no medianoche inventada.
- H/I scorecards materializan primeros aprobados y resultados; I normaliza una vez el subconjunto de historial, nombres y ligas. Caché API 2 minutos, lecturas concurrentes compartidas, corte original visible, errores sin caché y scope propio al desconectar pestañas. Última medición: I **17,731 s** y H **63,433 s**, repetición de ambos ≈1 ms. H sigue lento en frío; su bloque asíncrono espera hasta 90 s. G reciente **33,185 s**; histórico amplio sigue agotando 120 s. No dar por resuelta toda la lentitud.
- SQL final aplicado mediante transacciones limitadas a los objetos de lectura e índice revisados, con hashes de migración verificados; no se repitieron backfills históricos ni se modificaron picks. API/robots siguen en 5070; web en 5130. Log API: `/private/tmp/corners-api-20260921-labs-final.log`. El job `e0e1b0dc-d336-4ae9-92f0-fd4ce0517bd7` retomó el lote 3/11; no se creó otro job.
- Verificación: builds sin errores/advertencias; suites G/H/I, Web, liquidación y caché correctas. G incluye 11 casos SQL de probabilidades. `tests/BotILabResolution.Tests -- --sql` verifica 17 casos y cinco escenarios de equivalencia de todas las métricas (ventanas/versiones/duplicados/empates/cortes históricos), con rollback completo de esquema aislado.
- Próxima prioridad de datos: reconciliar IDs del **worker G**, ejecutar export/preflight v1.1 y cuantificar partidos aptos antes de entrenar. Completar las 224 identidades de I; analizar H usando también capturas rechazadas con resultados oficiales y un corte temporal fijado de antemano. El experimento de umbrales H sigue siendo exploratorio: su split cambia al cambiar filtros.
