"""Runtime autocontenido para inferencia con bundles CatBoost de conteos."""
...
class FootballCountDeploymentModel:
    ...
    def __init__(self, bundle_dir: str | Path, verify_checksums: bool = True) -> None:
        ...
        self.target = str(self.metadata.get("target"))
        if self.target not in SUPPORTED_TARGETS:
            raise ValueError(f"Target de conteo no soportado: {self.target!r}.")
...
# Alias estable para la primera integración publicada de Home Corners.
HomeCornersDeploymentModel = FootballCountDeploymentModel
